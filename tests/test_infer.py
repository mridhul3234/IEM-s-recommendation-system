...
from backend.infer import parse_acoustic_json
...
class TestInferTargetProfile:
    ...
    def test_gemini_success_path(self, monkeypatch):
        ...
        monkeypatch.setattr("backend.infer.call_gemini_api", lambda prompt, key: self._good_response())
        from backend.infer import infer_target_profile
    ...
    def test_gemini_failure_returns_default_without_ollama_delay(self, monkeypatch):
        monkeypatch.setenv("GEMINI_API_KEY", "AIzaSyFakeButFormatCorrect12345678")
        monkeypatch.setattr("backend.infer.call_gemini_api", lambda p, k: (_ for _ in ()).throw(Exception("API fail")))
        from backend.infer import infer_target_profile
    ...
    def test_both_fail_returns_default(self, monkeypatch):
        monkeypatch.setenv("GEMINI_API_KEY", "AIzaSyFakeButFormatCorrect12345678")
        monkeypatch.setattr("backend.infer.call_gemini_api", lambda p, k: (_ for _ in ()).throw(Exception("fail")))
        from backend.infer import infer_target_profile
        result = infer_target_profile("test")
        assert result == DEFAULT_PROFILE
